...
export default function ContextualWidgets({
    ...
    const loadStudyPlan = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.activePlan) {
            const firstIncomplete = data.activePlan.days.find(
              (d: StudyPlanDay) => !d.completed
            );
            if (firstIncomplete) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 409) {
          alert(
            'You already have an active plan. Complete or delete it first.'
          );
  ...
  const handleToggleComplete = async (day: StudyPlanDay) => {
    ...
    try {
      const response = await fetch(
        `/api/study-plans/${studyPlan.id}/progress`,
        {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      if (data.planCompleted) {
        ...
        setStudyPlan((prev) =>
          prev
            ? { ...prev, status: 'completed', completedAt: new Date() }
            : null
        );
  ...
  const handleSavePrayerFromPlan = async (day: StudyPlanDay) => {
    ...
    try {
      ...
      setHighlightedItems(
        (prev) => new Set([...prev, `prayer-${savedPrayer.id}`])
      );
      setTimeout(() => {
        setHighlightedItems((prev) => {
          const next = new Set(prev);
  ...
  const handleAskBereaAboutDay = async (day: StudyPlanDay) => {
    ...
    try {
      ...
      alert(
        'Study plan content copied to clipboard! Paste it into the chat below to discuss with Berea.'
      );
      ...
      alert(
        "Please scroll down to the chat and ask Berea about today's study content."
      );
    }
  };
    ...
    <>
      <div className={styles.widgetsContainer}>
        ...
        <div className={styles.widget}>
          <div
            className={styles.widgetHeader}
            onClick={() => toggleWidget('study')}
          >
            <div className={styles.widgetTitleRow}>
              <h3 className={styles.widgetTitle}>
                <span className={styles.widgetIcon}>📚</span> Study
              </h3>
              <button className={styles.chevronButton}>
                <svg
                  width="16"
                  ...
                  fill="none"
                  style={{
                    transform: collapsedWidgets.study
                      ? 'rotate(-90deg)'
                      : 'rotate(0deg)',
                  }}
                >
          ...
          <div
            className={`${styles.widgetContent} ${collapsedWidgets.study ? styles.collapsed : ''}`}
          >
            {isLoadingStudyPlan && (
              <div className={styles.creatingPlan}>
                <svg
            ...
            {!isLoadingStudyPlan && !studyPlan && !isCreatingPlan && (
              <div className={styles.studyPlanEmpty}>
                ...
                <p>
                  AI-personalized study plans tailored to your spiritual growth
                </p>
                <button
                  className={styles.createPlanButton}
                ...
                <div className={styles.planHeader}>
                  <div className={styles.planHeaderTop}>
                    ...
                    <div className={styles.planHeaderRight}>
                      ...
                      <span className={styles.planProgress}>
                        {studyPlan.days.filter((d) => d.completed).length}/
                        {studyPlan.days.length} days
                      </span>
                      <button
                  ...
                  {showPlanMenu && (
                    <div className={styles.planMenu}>
                      {studyPlan.status === 'completed' ? (
                        <button onClick={handleArchivePlan}>
                          Archive Plan
                        </button>
                      ) : (
                ...
                <div className={styles.progressBarContainer}>
                  <div className={styles.progressBar}>
                    <div
                      ...
                      style={{
                        width: `${(studyPlan.days.filter((d) => d.completed).length / studyPlan.days.length) * 100}%`,
                      }}
                    />
                  </div>
                </div>
                ...
                {showAllDays && (
                  <>
                    ...
                    {studyPlan.days.length > visibleDaysCount && (
                      ...
                      >
                        Show More ({studyPlan.days.length - visibleDaysCount}{' '}
                        more days)
                      </button>
                    )}
                ...
                {studyPlan.status === 'completed' && (
                  <div className={styles.archivePlanSection}>
                    <p className={styles.archiveMessage}>
                      Great work completing this journey! Ready to start a new
                      one?
                    </p>
                    <button
          ...
          >
            ...
            >
              ...
              <div className={styles.aiOption}>
                ...
                <p>
                  Custom study plan based on your conversations, verses, and
                  prayers
                </p>
                <div className={styles.durationButtons}>
                  <button
                    onClick={() => handleCreatePlan('ai_personalized', 7)}
                  >
                    7-Day Journey
                  </button>
                  <button
                    onClick={() => handleCreatePlan('ai_personalized', 21)}
                  >
                    21-Day Deep Dive
                  </button>
                </div>
              ...
              {TEMPLATE_OPTIONS.map((template) => (
                <div key={template.id} className={styles.templateOption}>
                  ...
                  <div className={styles.durationButtons}>
                    <button onClick={() => handleCreatePlan(template.id, 7)}>
                      7 Days
                    </button>
                    <button onClick={() => handleCreatePlan(template.id, 21)}>
                      21 Days
                    </button>
                  </div>
                </div>
        ...
        createPortal(
          <div className={styles.planCompletionModal}>
            <div className={styles.planCompletionContent}>
              ...
              <p>
                Congratulations on completing your {studyPlan?.duration}-day
                study plan!
              </p>
              <p className={styles.completionEncouragement}>
                You've taken meaningful steps in your spiritual growth. Keep
