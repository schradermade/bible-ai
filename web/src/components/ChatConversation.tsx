...
function VerseReference({
  ...
  return (
    ...
    >
      ...
      <div className={styles.verseTooltip}>
        <div>Save to Word</div>
      </div>
      {onSave && (
        isSaving ? (
          <svg
          ...
          <span role="img" aria-label="Save verse">📖</span>
        )
      )}
    </button>
  );
...
export default function ChatConversation({
  ...
  return (
    <div
      ref={containerRef}
      className={`${styles.chatContainer} ${messages.length > 0 ? styles.hasMessages : ''} ${!welcomeMinimized ? styles.showingWelcome : ''}`}
    >
      {!welcomeMinimized && (
      ...
      {welcomeMinimized && messages.length === 0 && (
        <div className={styles.newConversationPrompt}>
