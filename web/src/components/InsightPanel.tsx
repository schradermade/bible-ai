...
export default function InsightPanel({ content, isPreview = false }: InsightPanelProps) {
  ...
  const defaultContent = {
    ...
    mainInsight: 'God\'s love isn\'t based on our performance, but on His unchanging character. When we grasp this truth, it transforms how we approach Him—not with fear of rejection, but with confidence in His steadfast love.',
    scriptureContext: {
    ...
    application: 'Today, reflect on areas where you still approach God with performance-based thinking. His love for you is already complete—nothing you do can make Him love you more or less.',
    deeperTruth: 'The Father\'s heart toward you is one of complete acceptance and delight. Rest in this truth and let it reshape how you live, pray, and relate to Him.',
  };
  ...
  return (
    <div ref={panelRef} className={styles.insightPanel}>
      <div className={`${styles.header} ${!headerVisible ? styles.headerHidden : ''}`}>
        <h2 className={styles.panelTitle}>Insight</h2>
        <div className={styles.panelSubtitle}>Biblical Understanding for Today</div>
      </div>
      ...
      {displayContent.scriptureContext && (
        <>
          ...
          <div className={styles.contentSection}>
            ...
            <div className={styles.scriptureReference}>{displayContent.scriptureContext.reference}</div>
            <p className={styles.scriptureText}>"{displayContent.scriptureContext.text}"</p>
          </div>
        </>
      )}
